      ...
      <div className={`sticky top-0 z-20 backdrop-blur-xl ${darkMode ? 'bg-black/70 border-white/10' : 'bg-white/10 border-white/30'} border-b`}>
        <div className="max-w-md mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <h2 className={`${darkMode ? 'text-white' : 'text-[#9333ea]'} font-semibold text-xl`}>Settings</h2>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-md mx-auto px-4 py-6 space-y-6">
        {settingsSections.map((section) => (
          <div key={section.title}>
            <h3 className={`${darkMode ? 'text-gray-400' : 'text-[#555]'} font-medium text-sm px-2 mb-3`}>{section.title}</h3>
            <div className={`backdrop-blur-xl ${darkMode ? 'bg-white/10 border-white/20' : 'bg-white/10 border-white/30'} rounded-3xl border shadow-[0_8px_24px_rgba(139,92,246,0.12)] overflow-hidden`}>
        ...
        <div className="text-center pt-4">
          <p className={`${darkMode ? 'text-gray-500' : 'text-[#999]'} text-sm`}>Snag v1.0.0</p>
          <p className={`${darkMode ? 'text-gray-500' : 'text-[#999]'} text-xs mt-1`}>Made with 💜 for students</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div
      className={`w-full px-4 py-4 flex items-center gap-3 ${
        !isLast ? `border-b ${darkMode ? 'border-white/10' : 'border-purple-100/40'}` : ''
      }`}
    >
      <div className={`w-10 h-10 rounded-full ${darkMode ? 'bg-gradient-to-br from-purple-600/20 to-purple-700/20' : 'bg-gradient-to-br from-purple-100 to-purple-200'} flex items-center justify-center flex-shrink-0`}>
        <IconComponent className={`w-5 h-5 ${darkMode ? 'text-purple-400' : 'text-purple-600'}`} />
      </div>
      <span className={`${darkMode ? 'text-white' : 'text-[#222]'} font-medium flex-1`}>{item.label}</span>
      ...
      {item.hasToggle && item.onToggle ? (
        ...
        <button
          onClick={onClick}
          className={`${darkMode ? 'hover:bg-white/10' : 'hover:bg-white/50'} transition-colors active:scale-[0.98] p-2 rounded-full`}
        >
          <ChevronRight className={`w-5 h-5 ${darkMode ? 'text-gray-500' : 'text-[#999]'}`} />
        </button>
      )}
    </div>
  );
