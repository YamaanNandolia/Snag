...
export default function NotificationsScreen({ navigateTo }: any) {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            const data = snapshot.docs.map((doc) => ({
                id: doc.id,
                ...doc.data(),
                time: "Just now", // for now, all show as "Just now"
            }));
    ...
    // --- grouping logic (optional; everything will show under “Today” for now)
            ...
            <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                    <h3 className={`font-medium flex items-center gap-2 ${darkMode ? "text-white" : "text-[#222]"}`}>
                        {notification.title}
                        {notification.unread && <div className="w-2 h-2 rounded-full bg-red-500 flex-shrink-0" />}
                    </h3>
