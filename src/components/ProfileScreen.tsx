        ...
        <div className={`backdrop-blur-xl ${darkMode ? 'bg-white/10 border-white/20' : 'bg-white/10 border-white/30'} rounded-3xl p-6 border shadow-[0_8px_24px_rgba(139,92,246,0.12)] mb-6`}>
          ...
          <div className="flex items-start gap-4 mb-6">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <h2 className={`font-semibold ${darkMode ? 'text-white' : 'text-[#222]'}`}>Ryan Mehta</h2>
                <div className="w-5 h-5 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center">
                  <span className="text-white text-xs">✓</span>
                </div>
              </div>
              <p className={`mb-3 ${darkMode ? 'text-gray-400' : 'text-[#555]'}`}>@ryanmehta</p>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className={`text-center p-3 backdrop-blur-xl ${darkMode ? 'bg-white/5 border-white/10' : 'bg-purple-50/60 border-purple-200/40'} rounded-2xl border`}>
              <div className={`font-semibold mb-1 ${darkMode ? 'text-white' : 'text-[#222]'}`}>42</div>
              <div className={`text-xs font-medium ${darkMode ? 'text-gray-400' : 'text-[#555]'}`}>Credits</div>
            </div>
            <div className={`text-center p-3 backdrop-blur-xl ${darkMode ? 'bg-white/5 border-white/10' : 'bg-purple-50/60 border-purple-200/40'} rounded-2xl border`}>
              <div className={`font-semibold mb-1 ${darkMode ? 'text-white' : 'text-[#222]'}`}>12</div>
        ...
        <Tabs defaultValue="listings" className="w-full">
          ...
          <TabsContent value="listings" className="space-y-3">
            {myListings.length > 0 ? (
              <>
                {myListings.slice(0, 3).map((listing: any) => (
                  <button
                    ...
                    className={`w-full backdrop-blur-2xl ${darkMode ? 'bg-white/10 border-white/20 hover:bg-white/15' : 'bg-white/70 border-white/60 hover:shadow-[0_8px_24px_rgba(139,92,246,0.15)]'} rounded-2xl p-4 border shadow-sm transition-all active:scale-[0.98] text-left`}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <h3 className={`font-medium ${darkMode ? 'text-white' : 'text-[#222]'}`}>{listing.title}</h3>
                      <Badge
                        className={
                    ...
                    <div className={`flex items-center gap-4 text-sm ${darkMode ? 'text-gray-400' : 'text-[#555]'}`}>
                ...
                {myListings.length > 3 && (
                  <button
                    ...
                    className={`w-full py-3 rounded-2xl ${darkMode ? 'bg-white/10 text-purple-400 border-purple-400/60' : 'backdrop-blur-xl bg-white/80 text-purple-600 border-purple-200/60'} font-medium border shadow-sm transition-transform active:scale-[0.98] flex items-center justify-center gap-2`}
                  >
              ...
              <div className={`backdrop-blur-2xl ${darkMode ? 'bg-white/10 border-white/20' : 'bg-white/70 border-white/60'} rounded-3xl p-12 border shadow-[0_8px_24px_rgba(139,92,246,0.12)] text-center`}>
                ...
                <h3 className={`font-semibold mb-2 ${darkMode ? 'text-white' : 'text-[#222]'}`}>No listings yet</h3>
                <p className={`mb-4 ${darkMode ? 'text-gray-400' : 'text-[#555]'}`}>Start selling items to see them here</p>
